...
export default async function handler(req,res){
  ...
  if(requestMediaType(req)!=='application/json')return send(res,415,transportErrorBody(req,{code:-32600,message:'MCP POST requests require Content-Type: application/json'}));
  if(!acceptsMcpResponses(req))return send(res,406,transportErrorBody(req,{code:-32000,message:'Not Acceptable: MCP clients must accept both application/json and text/event-stream'}));
