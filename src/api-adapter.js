...
function supportedPrimitiveSerialization(parameter){const style=normalizedParameterStyle(parameter);if(parameter?.in==='path')return style==='simple'||style==='label'||style==='matrix';if(parameter?.in==='query')return style==='form';if(parameter?.in==='header')return style==='simple';return false;}
function groupSchema(parameters,location){const properties={},required=[];for(const parameter of Array.isArray(parameters)?parameters:[]){if(parameter?.in!==location||typeof parameter.name!=='string'||!parameter.name)continue;if(location==='header'&&FORBIDDEN_HEADERS.has(parameter.name.toLowerCase()))continue;const schema=primitiveParameterSchema(parameter.schema);if(!schema||!supportedPrimitiveSerialization(parameter))continue;properties[parameter.name]=schema;if(parameter.required)required.push(parameter.name);}if(!Object.keys(properties).length)return null;return{type:'object',properties,...(required.length?{required}:{}),additionalProperties:false};}
...
function operationBaseUrl(discovery,operation){const description=descriptionFor(discovery,operation);if(!description)return null;const servers=Array.isArray(operation?.servers)?operation.servers:description.servers;const raw=Array.isArray(servers)&&servers.length?servers[0]:'/';if(typeof raw!=='string'||raw.includes('{'))return null;return safeHttpUrl(raw,description.url);}
...
function hasUnsupportedRequiredParameter(operation){if(operation?.hasUnresolvedRequiredInputs)return true;for(const parameter of Array.isArray(operation.parameters)?operation.parameters:[]){if(!parameter?.required)continue;if(!PARAM_LOCATIONS.has(parameter.in))return true;if(parameter.in==='header'&&FORBIDDEN_HEADERS.has(String(parameter.name).toLowerCase()))return true;if(!primitiveParameterSchema(parameter.schema)||!supportedPrimitiveSerialization(parameter))return true;}if(operation.requestBody?.required&&operation.requestBody?.contentType!=='application/json')return true;return false;}
function pathSerializationFor(operation){const out={};for(const parameter of Array.isArray(operation?.parameters)?operation.parameters:[]){if(parameter?.in!=='path'||typeof parameter.name!=='string'||!parameter.name||!primitiveParameterSchema(parameter.schema)||!supportedPrimitiveSerialization(parameter))continue;out[parameter.name]=normalizedParameterStyle(parameter);}return out;}
...
function appendQuery(url,name,value){if(value===undefined)return;if(['string','number','boolean'].includes(typeof value)){url.searchParams.append(name,String(value));return;}throw new TypeError(`Unsupported query value for ${name}`);}
...
export function previewOpenApiRequest(candidate,args={}){
  ...
  const url=safeHttpUrl(urlText);if(!url)throw new TypeError('Candidate URL is not a safe HTTP(S) URL');const querySchema=schema.properties?.query;
