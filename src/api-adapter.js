...
function supportedParameterSerialization(parameter){const style=normalizedParameterStyle(parameter),schema=supportedParameterSchema(parameter);if(!schema)return false;if(schema.type!=='array'){if(parameter?.in==='path')return style==='simple'||style==='label'||style==='matrix';if(parameter?.in==='query')return style==='form';if(parameter?.in==='header')return style==='simple';return false;}if(parameter?.in!=='query')return false;const explode=normalizedExplode(parameter);if(style==='form')return true;if(style==='spaceDelimited'||style==='pipeDelimited')return explode===false;return false;}
...
function appendQuery(url,name,value,serialization={style:'form',explode:true,type:'string'}){if(value===undefined)return;if(serialization.type!=='array'){if(['string','number','boolean'].includes(typeof value)){url.searchParams.append(name,String(value));return;}throw new TypeError(`Unsupported query value for ${name}`);}if(!Array.isArray(value))throw new TypeError(`Query parameter ${name} must be an array`);for(const item of value)if(!['string','number','boolean'].includes(typeof item))throw new TypeError(`Unsupported query array value for ${name}`);if(serialization.style==='form'&&serialization.explode){for(const item of value)url.searchParams.append(name,String(item));return;}const delimiter=serialization.style==='form'?',':serialization.style==='spaceDelimited'?' ':serialization.style==='pipeDelimited'?'|':null;if(delimiter===null)throw new TypeError(`Unsupported query serialization style for ${name}: ${serialization.style}`);url.searchParams.append(name,value.map(String).join(delimiter));}
