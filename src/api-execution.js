...
const EXECUTABLE_SCHEMA_KEYS=new Set(['type','enum','properties','required','additionalProperties','items','allOf','minItems','maxItems','minLength','maxLength','pattern','multipleOf','minimum','maximum','title','description','default','examples','example','format','deprecated','readOnly','writeOnly']);
...
function assertExecutableSchemaSubset(schema,path='$'){if(!schema||typeof schema!=='object'||Array.isArray(schema))throw new TypeError(`Execution schema is not enforceable at ${path}`);for(const key of Object.keys(schema))if(!EXECUTABLE_SCHEMA_KEYS.has(key))throw new TypeError(`Execution schema keyword is not supported at ${path}: ${key}`);if(schema.type!==undefined){const types=Array.isArray(schema.type)?schema.type:[schema.type];const valid=(typeof schema.type==='string'||Array.isArray(schema.type))&&types.length>0&&types.every(type=>typeof type==='string'&&EXECUTABLE_SCHEMA_TYPES.has(type))&&new Set(types).size===types.length;if(!valid)throw new TypeError(`Invalid type schema at ${path}`);}if(schema.additionalProperties!==undefined&&typeof schema.additionalProperties!=='boolean')throw new TypeError(`Schema-valued additionalProperties is not supported for execution at ${path}`);if(schema.multipleOf!==undefined&&(typeof schema.multipleOf!=='number'||!Number.isFinite(schema.multipleOf)||schema.multipleOf<=0))throw new TypeError(`Invalid multipleOf schema at ${path}`);if(schema.pattern!==undefined){if(typeof schema.pattern!=='string'||schema.pattern.length>MAX_PATTERN_LENGTH)throw new TypeError(`Invalid pattern schema at ${path}`);try{new RegExp(schema.pattern,'u');}catch{throw new TypeError(`Invalid pattern schema at ${path}`);}}if(schema.allOf!==undefined){if(!Array.isArray(schema.allOf)||!schema.allOf.length||schema.allOf.length>MAX_ALLOF_BRANCHES)throw new TypeError(`Invalid or oversized allOf schema at ${path}`);for(let i=0;i<schema.allOf.length;i+=1)assertExecutableSchemaSubset(schema.allOf[i],`${path}.allOf[${i}]`);}if(schema.properties!==undefined){if(!schema.properties||typeof schema.properties!=='object'||Array.isArray(schema.properties))throw new TypeError(`Invalid properties schema at ${path}`);for(const [name,child] of Object.entries(schema.properties))assertExecutableSchemaSubset(child,`${path}.properties.${name}`);}if(schema.items!==undefined)assertExecutableSchemaSubset(schema.items,`${path}.items`);}
