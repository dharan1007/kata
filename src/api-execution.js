...
const MAX_SCHEMA_BRANCHES=32;
const MAX_PATTERN_LENGTH=1024;
const EXECUTABLE_SCHEMA_KEYS=new Set(['type','enum','const','properties','required','additionalProperties','items','prefixItems','allOf','anyOf','oneOf','minItems','maxItems','minLength','maxLength','pattern','multipleOf','minimum','maximum','exclusiveMinimum','exclusiveMaximum','title','description','default','examples','example','format','deprecated','readOnly','writeOnly']);
...
function assertExecutableSchemaSubset(schema,path='$'){if(!schema||typeof schema!=='object'||Array.isArray(schema))throw new TypeError(`Execution schema is not enforceable at ${path}`);for(const key of Object.keys(schema))if(!EXECUTABLE_SCHEMA_KEYS.has(key))throw new TypeError(`Execution schema keyword is not supported at ${path}: ${key}`);if(schema.type!==undefined){const types=Array.isArray(schema.type)?schema.type:[schema.type];const valid=(typeof schema.type==='string'||Array.isArray(schema.type))&&types.length>0&&types.every(type=>typeof type==='string'&&EXECUTABLE_SCHEMA_TYPES.has(type))&&new Set(types).size===types.length;if(!valid)throw new TypeError(`Invalid type schema at ${path}`);}if(schema.additionalProperties!==undefined){if(typeof schema.additionalProperties==='boolean'){}else if(schema.additionalProperties&&typeof schema.additionalProperties==='object'&&!Array.isArray(schema.additionalProperties))assertExecutableSchemaSubset(schema.additionalProperties,`${path}.additionalProperties`);else throw new TypeError(`Invalid additionalProperties schema at ${path}`);}if(schema.multipleOf!==undefined&&(typeof schema.multipleOf!=='number'||!Number.isFinite(schema.multipleOf)||schema.multipleOf<=0))throw new TypeError(`Invalid multipleOf schema at ${path}`);for(const keyword of ['exclusiveMinimum','exclusiveMaximum'])if(schema[keyword]!==undefined&&(typeof schema[keyword]!=='number'||!Number.isFinite(schema[keyword])))throw new TypeError(`Invalid ${keyword} schema at ${path}`);if(schema.pattern!==undefined){if(typeof schema.pattern!=='string'||schema.pattern.length>MAX_PATTERN_LENGTH)throw new TypeError(`Invalid pattern schema at ${path}`);try{new RegExp(schema.pattern,'u');}catch{throw new TypeError(`Invalid pattern schema at ${path}`);}}for(const keyword of ['allOf','anyOf','oneOf']){if(schema[keyword]===undefined)continue;if(!Array.isArray(schema[keyword])||!schema[keyword].length||schema[keyword].length>MAX_SCHEMA_BRANCHES)throw new TypeError(`Invalid or oversized ${keyword} schema at ${path}`);for(let i=0;i<schema[keyword].length;i+=1)assertExecutableSchemaSubset(schema[keyword][i],`${path}.${keyword}[${i}]`);}if(schema.prefixItems!==undefined){if(!Array.isArray(schema.prefixItems)||!schema.prefixItems.length||schema.prefixItems.length>MAX_SCHEMA_BRANCHES)throw new TypeError(`Invalid or oversized prefixItems schema at ${path}`);for(let i=0;i<schema.prefixItems.length;i+=1)assertExecutableSchemaSubset(schema.prefixItems[i],`${path}.prefixItems[${i}]`);}if(schema.properties!==undefined){if(!schema.properties||typeof schema.properties!=='object'||Array.isArray(schema.properties))throw new TypeError(`Invalid properties schema at ${path}`);for(const [name,child] of Object.entries(schema.properties))assertExecutableSchemaSubset(child,`${path}.properties.${name}`);}if(schema.items!==undefined){if(typeof schema.items==='boolean'){}else assertExecutableSchemaSubset(schema.items,`${path}.items`);}}
