...
const annotations={
  browserSearch:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:true},
  readLocal:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
  runAutomation:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:true},
  executeLearned:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:false}
};
export function createWebMcpRegistry(runtime,onStatus=()=>{}){
  let controller=null;
  const builtins=()=>[
    {name:'kata_search_research',description:'Search live OpenAlex research and load the visible KATA workspace.',inputSchema:{type:'object',properties:{query:{type:'string'},limit:{type:'integer',minimum:1,maximum:25}},required:['query'],additionalProperties:false},annotations:annotations.browserSearch,execute:({query,limit=8},context={})=>runtime.search(query,limit,'agent',{signal:context.signal})},
    {name:'kata_workspace_summary',description:'Read the current browser-owned KATA workspace summary.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:annotations.readLocal,execute:()=>runtime.summary()},
    {name:'kata_list_automations',description:'List current KATA automations.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:annotations.readLocal,execute:()=>runtime.listAutomations()},
    {name:'kata_run_automation',description:'Preview and run a saved KATA automation against current candidates.',inputSchema:{type:'object',properties:{automationId:{type:'string'}},required:['automationId'],additionalProperties:false},annotations:annotations.runAutomation,execute:({automationId},context={})=>runtime.runAutomation(automationId,'agent',undefined,{signal:context.signal})},
    {name:'kata_list_learned_tools',description:'List deterministic tools learned in this browser.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:annotations.readLocal,execute:()=>runtime.listPrograms()}
  ];
  async function refresh(){
    ...
    const tools=builtins();for(const p of runtime.listPrograms())tools.push({name:p.name,description:p.description,inputSchema:p.inputSchema,annotations:annotations.executeLearned,execute:(input,context={})=>runtime.executeProgram(p.name,input,'agent',{signal:context.signal})});
