...
import {MCP_VERSION} from '../lib/server/mcp.js';
...
test('/api/openapi publishes the modern MCP transport contract',async()=>{
  ...
  assert.equal(parameters['MCP-Protocol-Version'].schema.const,MCP_VERSION);
  ...
  assert.equal(operation.responses['202'].description,'Accepted modern MCP notification; no JSON-RPC response body');
