...
function chromeApi(executionResult={ok:true,status:200,statusText:'OK',url:'https://app.test/api/items/a%2Fb',contentType:'application/json',bytes:11,bodyText:'{"ok":true}',truncated:false}){
  const calls=[];
...
test('page executor enforces origin, timeout/response bounds, no redirects, and browser-managed credentials without exposing cookies',async()=>{
  assert.equal(typeof worker.executePageApiRequest,'function');
  const seen=[];
  const runtime={location:{origin:'https://app.test'},fetch:async(url,init)=>{seen.push({url,init});return{ok:true,status:200,statusText:'OK',url,headers:{get(name){if(name.toLowerCase()==='content-type')return'application/json';if(name.toLowerCase()==='content-length')return'11';return null;}},body:null,async text(){return'{"ok":true}';}};},AbortController,Uint8Array,TextDecoder,setTimeout,clearTimeout,performance:{now:()=>10}};
  ...
  await assert.rejects(()=>worker.executePageApiRequest({...request,maxResponseBytes:5},runtime),/response.*large/i);
});
