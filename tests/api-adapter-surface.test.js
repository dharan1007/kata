...
test('capabilities distinguish local OpenAPI compilation from user-authorized preview-bound execution',async()=>{
  ...
  assert.deepEqual(execution,{source:'standards-discovered-openapi',mode:'user-authorized-preview-bound',sameOriginOnly:true,executionWorld:'MAIN',rediscoversContractBeforeExecution:true,sha256PreviewBinding:true,credentialModes:['omit','same-origin-browser-managed'],credentialPolicy:'declared-openapi-security-scheme-bound',preservesSecurityRequirementAlternatives:true,preservesAndRequirements:true,authSchemeBinding:'per-description',browserManagedAuth:['apiKey-cookie'],unsupportedBrowserManagedAuth:['apiKey-header','apiKey-query','http','oauth2','openIdConnect','mutualTLS'],anonymousSecurityAlternativeUsesCredentials:'omit',extractsCredentials:false,acceptsCredentialArguments:false,redirects:'error',automaticRetries:false,defaultTimeoutMs:15000,maxTimeoutMs:15000,timeoutScope:'request-and-response-body',maxRequestUrlBytes:16384,maxRequestHeaderBytes:32768,maxRequestBodyBytes:262144,defaultMaxResponseBytes:1048576,maxResponseBytes:1048576,responseMode:'bounded-snapshot',oversizedDeclaredResponse:'cancel-and-abort-download',indeterminateNetworkOutcome:'unknown-no-auto-retry',receipts:'local-only',explicitExecutionApproval:true});
