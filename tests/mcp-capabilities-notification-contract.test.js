...
test('/api/capabilities requires the modern routing headers for notifications as well as requests',async()=>{
  const r=res();
  ...
  assert.deepEqual(contract.notificationRequiredHeaders,['Content-Type','Accept','MCP-Protocol-Version','Mcp-Method']);
  assert.equal(contract.notificationRoutingHeadersRequired,true);
});
