...
test('/api/capabilities publishes canonical WebMCP parity, runtime/API discovery, MCP inspection and bounded active-tab execution',async()=>{
  ...
  assert.deepEqual(interop.mcpToolAdapters,{source:'user-authorized-active-tab',protocol:'2026-07-28',transport:'streamable-http',discoveryMethod:'tools/list',executionMethod:'tools/call',sameOriginOnly:true,credentials:'omit',supportsJsonResponses:true,supportsSseResponses:true,maxPages:8,maxTools:100,maxResponseBytes:1048576,validatesXMcPHeader:true,mirrorsMcpParamHeaders:true,encodesUnsafeMcpParamValues:'base64-sentinel',sha256PreviewBinding:true,relistBeforeExecution:true,explicitExecutionApproval:true,automaticRetries:false,fullJsonSchema202012Execution:false,executableInputSchemaSubset:'bounded',structuredOutputValidation:'supported-subset-only',supportsInputRequired:false,supportsTasksExtension:false,startsOAuth:false,legacyExecution:false,protectedServerExecution:false,receipts:'local-only'});
  assert.deepEqual(interop.browserExtension,{mode:'manifest-v3-active-tab',repositoryPath:'extension',builtArtifactPath:'dist/extension',minimumChromeVersion:95,permissions:['activeTab','scripting'],hostPermissions:['https://kata-webmcp.vercel.app/*'],userGestureRequired:true,executionWorld:'MAIN',canonicalProbe:'inspectBrowserRuntime',topFrameOnly:true,supportedIntents:['read','act','automate','expose_webmcp','call_api','connect_mcp'],outboundToKata:['intent','environment'],localOnly:['url','origin','frameworkHints','domTopology','declaredApiDescriptions','compiledApiToolContracts','apiExecutionPreviews','apiExecutionReceipts','mcpEndpoint','mcpServerInfo','mcpAuthorizationMetadata','mcpToolContracts','mcpToolPreviews','mcpToolReceipts','evidence'],persistentThirdPartyHostAccess:false,apiExecution:{source:'standards-discovered-openapi',mode:'user-authorized-preview-bound',sameOriginOnly:true,executionWorld:'MAIN',rediscoversContractBeforeExecution:true,sha256PreviewBinding:true,credentialModes:['omit','same-origin-browser-managed'],extractsCredentials:false,acceptsCredentialArguments:false,redirects:'error',automaticRetries:false,defaultTimeoutMs:15000,maxTimeoutMs:15000,maxRequestUrlBytes:16384,maxRequestHeaderBytes:32768,maxRequestBodyBytes:262144,defaultMaxResponseBytes:1048576,maxResponseBytes:1048576,responseMode:'bounded-snapshot',indeterminateNetworkOutcome:'unknown-no-auto-retry',receipts:'local-only',explicitExecutionApproval:true},mcpExecution:{mode:'user-authorized-preview-bound',modernProtocolOnly:true,sameOriginOnly:true,credentials:'omit',redirects:'manual',automaticRetries:false,relistBeforeExecution:true,sha256PreviewBinding:true,explicitExecutionApproval:true,maxResponseBytes:1048576,supportsSseResponses:true,structuredOutputValidation:'supported-subset-only',supportsInputRequired:false,supportsTasksExtension:false,startsOAuth:false,legacyExecution:false,receipts:'local-only'}});
});
