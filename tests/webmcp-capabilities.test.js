...
test('/api/capabilities publishes canonical WebMCP parity, runtime/API discovery and constrained MCP endpoint inspection',async()=>{
  ...
  assert.deepEqual(webmcp.apiAgentAdapters,{tool:'kata_browser_compile_api_tools',source:'standards-discovered-openapi',mode:'preview-only',resolvesLocalComponentRefs:true,fetchesExternalRefs:false,acceptsCredentialArguments:false,executesOperations:false,maxTools:100});
  ...
  assert.deepEqual(interop.browserExtension,{mode:'manifest-v3-active-tab',repositoryPath:'extension',builtArtifactPath:'dist/extension',minimumChromeVersion:95,permissions:['activeTab','scripting'],hostPermissions:['https://kata-webmcp.vercel.app/*'],userGestureRequired:true,executionWorld:'MAIN',canonicalProbe:'inspectBrowserRuntime',topFrameOnly:true,supportedIntents:['read','act','automate','expose_webmcp','call_api','connect_mcp'],outboundToKata:['intent','environment'],localOnly:['url','origin','frameworkHints','domTopology','declaredApiDescriptions','compiledApiToolContracts','mcpEndpoint','mcpServerInfo','mcpAuthorizationMetadata','evidence'],persistentThirdPartyHostAccess:false});
});
