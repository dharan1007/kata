...
test('/api/capabilities publishes canonical WebMCP parity, runtime/API discovery and constrained MCP endpoint inspection',async()=>{
  ...
  assert.deepEqual(webmcp.browserTools,['kata_browser_inspect_runtime','kata_browser_discover_api','kata_browser_search_and_load_research','kata_browser_workspace_summary','kata_browser_list_automations','kata_browser_run_saved_automation','kata_browser_list_learned_tools']);
  ...
  assert.deepEqual(interop.browserExtension,{mode:'manifest-v3-active-tab',repositoryPath:'extension',builtArtifactPath:'dist/extension',minimumChromeVersion:95,permissions:['activeTab','scripting'],hostPermissions:['https://kata-webmcp.vercel.app/*'],userGestureRequired:true,executionWorld:'MAIN',canonicalProbe:'inspectBrowserRuntime',topFrameOnly:true,supportedIntents:['read','act','automate','expose_webmcp','call_api','connect_mcp'],outboundToKata:['intent','environment'],localOnly:['url','origin','frameworkHints','domTopology','declaredApiDescriptions','mcpEndpoint','mcpServerInfo','mcpAuthorizationMetadata','evidence'],persistentThirdPartyHostAccess:false});
});
