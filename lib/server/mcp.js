...
function isLegacyEnvelope(request,body){
  const protocol=header(request.headers,'mcp-protocol-version');
  return protocol===LEGACY_MCP_VERSION||(protocol===undefined&&body?.method==='initialize');
}

function invalidJsonRpc(request,body,message='Invalid JSON-RPC request'){
  const legacy=isLegacyEnvelope(request,body);
...
export async function handleMcpRequest(request,options={}){
  ...
  try{
    const protocol=header(request.headers,'mcp-protocol-version');
    if(method==='initialize'&&(protocol===undefined||protocol===LEGACY_MCP_VERSION)){
    ...
    if(protocol===MCP_VERSION)return await handleModern(request,body,registry,env,ctx);
