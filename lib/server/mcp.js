...
async function invokeTool(registry,body,legacy){
  ...
  try{
    ...
    if(error?.message==='TOOL_NOT_FOUND')throw error;
    result={error:error instanceof Error?error.message:String(error),...(error?.details!==undefined?{details:error.details}:{})};
