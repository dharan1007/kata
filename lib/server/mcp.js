...
import {invalidMcpTraceContextField} from './trace-context.js';
...
const MCP_MODERN_SUPPORTED_VERSIONS=[MCP_VERSION];
...
function unsupportedVersion(body,legacy=false,requestedVersion){
  ...
  return{status:400,body:modernError(body?.id??null,UNSUPPORTED_PROTOCOL_VERSION,'Unsupported MCP protocol version',{supported:MCP_MODERN_SUPPORTED_VERSIONS,requested})};
}
...
function traceContextError(body,legacy,field){
  return{status:400,body:(legacy?legacyError:modernError)(body.id??null,-32602,'Invalid MCP trace context',{field})};
}

async function handleLegacy(body,registry,ctx,traceField){
  if(body.id===undefined)return{status:202,body:undefined};
  if(traceField)return traceContextError(body,true,traceField);
...
async function handleModern(request,body,registry,env,ctx,traceField){
  const notification=body.id===undefined;
  const envelopeError=validateModernEnvelope(request,body,{validateHeaders:!notification});
  ...
  if(traceField)return traceContextError(body,false,traceField);

  const cacheScope=metadataCacheScope(env),ttlMs=metadataTtlMs(env),cacheControl=metadataCacheControl(env);
  if(method==='server/discover')return{status:200,body:modernRpc(body.id,{supportedVersions:MCP_MODERN_SUPPORTED_VERSIONS,capabilities:{tools:{}},instructions:INSTRUCTIONS,ttlMs,cacheScope}),headers:{'Cache-Control':cacheControl}};
...
export async function handleMcpRequest(request,options={}){
  ...
  const traceField=invalidMcpTraceContextField(requestMeta);
  const ctx={signal:options.signal,meta:traceField?{}:requestMeta&&typeof requestMeta==='object'&&!Array.isArray(requestMeta)?structuredClone(requestMeta):{}};
  ...
  try{
    ...
    if(modernClaim)return await handleModern(request,body,registry,env,ctx,traceField);
    if(protocol===LEGACY_MCP_VERSION)return await handleLegacy(body,registry,ctx,traceField);
