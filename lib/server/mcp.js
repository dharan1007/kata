...
async function handleModern(request,body,registry){
  ...
  if(!routed||routed!==method)return{status:400,body:modernError(body.id??null,-32600,'Mcp-Method header does not match JSON-RPC method')};
  const name=body.params?.name;
  const routedName=header(request.headers,'mcp-name');
  if(method==='tools/call'&&(!routedName||routedName!==name))return{status:400,body:modernError(body.id??null,-32602,'Mcp-Name header does not match tool name')};
