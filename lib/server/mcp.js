...
async function handleModern(request,body,registry,env){
  ...
  if(method==='server/discover')return{status:200,body:modernRpc(body.id,{supportedVersions:MCP_SUPPORTED_VERSIONS,capabilities:{tools:{}},instructions:INSTRUCTIONS,ttlMs:60000,cacheScope:'public'}),headers:{'Cache-Control':metadataCacheControl(env)}};
  if(method==='tools/list')return{status:200,body:modernRpc(body.id,{tools:registry.list(),ttlMs:60000,cacheScope:'public'}),headers:{'Cache-Control':metadataCacheControl(env)}};
...
export async function handleMcpRequest(request,options={}){
  ...
  try{
    ...
    const protocol=header(request.headers,'mcp-protocol-version');
    if(protocol===LEGACY_MCP_VERSION)return await handleLegacy(body,registry);
    if(protocol===MCP_VERSION)return await handleModern(request,body,registry,env);
