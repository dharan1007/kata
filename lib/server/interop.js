...
export function diagnoseWebInterop({intent,environment:e}){
  ...
  const crossOriginRequestMissing=crossOriginFrame&&e.crossOriginRequest==='not-requested';
  const crossOriginPermissionUnverified=crossOriginFrame&&e.toolsPermission==='unknown';
  const crossOriginExposureUnverified=crossOriginFrame&&(e.originExposure==='unknown'||e.originExposure==='not-required');
  const crossOriginRequestUnverified=crossOriginFrame&&(e.crossOriginRequest==null||e.crossOriginRequest==='unknown'||e.crossOriginRequest==='not-required');
  const crossOriginWebMcpUnverified=crossOriginPermissionUnverified||crossOriginExposureUnverified||crossOriginRequestUnverified;
  ...
  if(e.webMcpApi==='available'){
    ...
    if(e.toolsPermission==='blocked'){
      webMcpBlocked=true;
      add(blockers,'WEBMCP_PERMISSION_POLICY','WebMCP tool registration is disabled in the observed document by the tools Permissions Policy.',crossOriginFrame?'Have the embedding origin explicitly delegate the tools capability, for example with iframe allow="tools" or an equivalent Permissions-Policy configuration.':'When you control the document policy, allow the tools capability instead of sending or inheriting a restrictive Permissions-Policy such as tools=(). Otherwise use a supported integration path that does not require changing the site security policy.');
    ...
    if(crossOriginRequestMissing){
      webMcpBlocked=true;
      add(blockers,'WEBMCP_FROM_ORIGINS_REQUIRED','The consuming document has not explicitly requested tools from the cross-origin producer, so compliant WebMCP discovery will not return those tools.','Request the producer origin explicitly with document.modelContext.getTools({fromOrigins:["https://producer.example"]}) using the actual secure producer origin; keep the existing tools Permissions Policy and producer exposedTo restrictions in place.');
      if(!hasDocumentedApi&&status!=='blocked')status='setup_required';
    }
    if(crossOriginWebMcpUnverified){
      webMcpBlocked=true;
      add(blockers,'WEBMCP_CROSS_ORIGIN_UNVERIFIED','Cross-origin WebMCP cannot be declared usable until the required permission delegation, producer origin exposure, and consumer origin request are all positively established.','Verify that the cross-origin frame is delegated the tools feature, the producer explicitly exposes tools to the consuming secure origin, and the consumer requests that producer with document.modelContext.getTools({fromOrigins:[...]}). Do not assume an unknown or not-required cross-origin control is satisfied.');
      if(!hasDocumentedApi&&status!=='blocked')status='setup_required';
    }
