...
export async function searchOpenAlex(query,limit=8,options={}){
  ...
  for(let attempt=0;;attempt++){
    ...
    if(res.status>=500&&attempt<retries){await sleepImpl(retryDelay(attempt));continue;} if(res.status===429)throw rateLimitError(res.headers); if(!res.ok)throw new Error('UPSTREAM_UNAVAILABLE');
