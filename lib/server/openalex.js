...
export async function searchOpenAlex(query,limit=8,options={}){
  ...
  for(let attempt=0;;attempt++){
    ...
    try{res=await fetchImpl(url,{signal:controller.signal,headers});}catch(error){clearTimeout(timer); if(controller.signal.aborted)throw new Error('UPSTREAM_TIMEOUT'); if(attempt<retries){await sleepImpl(retryDelay(attempt));continue;} throw new Error('UPSTREAM_UNAVAILABLE');} clearTimeout(timer);
    if((res.status===429||res.status>=500)&&attempt<retries){await sleepImpl(retryDelay(attempt));continue;} if(res.status===429)throw rateLimitError(res.headers); if(!res.ok)throw new Error('UPSTREAM_UNAVAILABLE');
