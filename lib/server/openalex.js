...
function rateLimitError(headers){const error=new Error('UPSTREAM_RATE_LIMITED');const resetSeconds=numericHeader(headers,'x-ratelimit-reset');if(resetSeconds!=null)error.details={retryAfterSeconds:Math.max(0,resetSeconds)};return error;}
...
const sleep=(ms,signal)=>new Promise((resolve,reject)=>{
  ...
  const onAbort=()=>{if(settled)return;settled=true;clearTimeout(timer);cleanup();reject(cancelledError());};
  const timer=setTimeout(finish,ms);
  signal?.addEventListener?.('abort',onAbort,{once:true});
});
export async function searchOpenAlex(query,limit=8,options={}){
  const fetchImpl=options.fetchImpl??fetch, sleepImpl=options.sleepImpl??sleep, retries=options.retries??2, retryDelayMs=options.retryDelayMs??150, timeoutMs=options.timeoutMs??8000, url=buildOpenAlexUrl(query,limit);
  ...
  const retryDelay=attempt=>retryDelayMs*(2**attempt);
  for(let attempt=0;;attempt++){
    const timeoutSignal=AbortSignal.timeout(timeoutMs);
    const signal=options.signal?AbortSignal.any([options.signal,timeoutSignal]):timeoutSignal;
    let res;
    try{res=await fetchImpl(url,{signal,headers});}catch(error){if(options.signal?.aborted)throw cancelledError(); if(timeoutSignal.aborted)throw new Error('UPSTREAM_TIMEOUT'); if(attempt<retries){await sleepImpl(retryDelay(attempt),options.signal);continue;} throw new Error('UPSTREAM_UNAVAILABLE');}
    if(res.status>=500&&attempt<retries){await sleepImpl(retryDelay(attempt),options.signal);continue;} if(res.status===429)throw rateLimitError(res.headers); if(!res.ok)throw new Error('UPSTREAM_UNAVAILABLE');
