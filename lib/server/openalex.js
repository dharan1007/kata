...
export async function searchOpenAlex(query,limit=8,options={}){
  ...
  for(let attempt=0;;attempt++){
    ...
    let res;
    try{res=await fetchImpl(url,{signal,headers});}catch(error){if(options.signal?.aborted)throw new Error('REQUEST_CANCELLED'); if(timeoutSignal.aborted)throw new Error('UPSTREAM_TIMEOUT'); if(attempt<retries){await sleepImpl(retryDelay(attempt));continue;} throw new Error('UPSTREAM_UNAVAILABLE');}
