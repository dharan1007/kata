...
export async function searchOpenAlex(query,limit=8,options={}){
  ...
  for(let attempt=0;;attempt++){
    ...
    if((res.status===429||res.status>=500)&&attempt<retries){await sleepImpl(retryDelay(attempt));continue;} if(res.status===429)throw new Error('UPSTREAM_RATE_LIMITED'); if(!res.ok)throw new Error('UPSTREAM_UNAVAILABLE');
