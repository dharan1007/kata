...
function decision(paths,intent){const priority=priorityFor(intent);for(const desired of ['possible','setup_required','blocked','unknown']){const candidates=priority.map(id=>paths[id]).filter(p=>p.status===desired);if(candidates.length)return{status:desired==='unknown'?'unknown':desired,primaryPath:candidates[0].id};}return{status:'unknown',primaryPath:priority.at(-1)??'user_authorized_browser'};}
