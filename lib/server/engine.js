...
export function applyCommand(workspace,command){
  ...
  const known=next.knownWorks[workId], saved=next.savedWorks[workId];
  if(kind==='SAVE_WORK'){
...
function matches(work,a,ws){if(a.filters.minYear!=null&&(work.year??0)<a.filters.minYear)return false;if(a.filters.minCitations!=null&&(work.citations??0)<a.filters.minCitations)return false;if(a.filters.onlyUnsaved&&ws.savedWorks[work.id])return false;return true;}
